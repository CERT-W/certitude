...
logger = logging.getLogger('RemoteCmd')
...
class RemoteCmd:
	...
	def __log__(self, debugLevel, message, exception=''):
		...
		m = '%s@%s %s' % (self.__login, self.__ip, message)
		if exception!='':
			m += ' (%s)' % str(exception)
			
		logger.log(debugLevel, m)

		if debugLevel >= LEVEL_CRITICAL:
			self.__del__(True)
	...
	#   Does the setup
	#
	def __init__(self, ip, login, password, **kwargs):
	...
	def __setup(self,ip, login, password, domain = ''):
		...
		if not self.fileExists(RemoteCmd.REMCOMSVC_REMOTE):
			lpath = os.path.join(self.rootDir, RemoteCmd.REMCOMSVC_LOCAL)
			self.dropFile( lpath, RemoteCmd.REMCOMSVC_REMOTE)
			self.__log__(LEVEL_INFODBG, 'RemCom service binary dropped')
		else:
			self.__log__(LEVEL_WARNING, 'RemCom service binary already here')
	...
	#
	def __del__(self, raiseit = False):

		try:
			self.__unsetup()
				
		except smbconnection.SessionError, e:
			print '[CRITICAL] Cleanup error: '+str(e)
			raise
		
		if raiseit:
				raise Exception('Program terminated - Cleanup successful')
	...
	#   remote share.
	#
	def dropFile(self, localFile, remoteName, useWd=True):
		...
		try:
			...
			self.__log__(LEVEL_CRITICAL, 'File '+localFile+' was not found')
	...
	def getFile(self, remoteName, localFile, useWd=True):
		...
		try:
			...
			self.__log__(LEVEL_CRITICAL, 'File '+localFile+' could not be created')
	...
	def unsetNet(self):
		...
		self.__log__(LEVEL_INFODBG, 'Net unuse: '+out.replace('\n',''))
